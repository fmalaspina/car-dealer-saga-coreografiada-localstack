...
public class SqsPoller {
  ...
  void handle(Message m){ try {
      ...
      if ("UnitReserved".equals(body.get("type").asText())){
        bus.publish(new EventBus.Event("PaymentConfirmed", body.get("orderId").asText()));
      } else if ("ReservationFailed".equals(body.get("type").asText())){
        System.out.println("[billing] no se cobra, reserva fallida: " + body.get("orderId").asText());
      }
